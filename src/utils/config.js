...
export const config = {
  discord: {
    ...
    controlChannelId: process.env.DISCORD_CONTROL_CHANNEL_ID ||
      fileConfig.discord?.controlChannelId,
  ...
  minecraft: {
    ...
    version: process.env.MC_VERSION || fileConfig.minecraft?.version ||
      undefined,
    loginCommandDelayMs: Number(
      process.env.MC_LOGIN_COMMAND_DELAY_MS ||
        fileConfig.minecraft?.loginCommandDelayMs || 3000,
    ),
  },
  intervals: {
    statusUpdate: Number(
      process.env.STATUS_UPDATE_INTERVAL_MS ||
        fileConfig.intervals?.statusUpdate || 5000,
    ),
  },
  features: {
    statusMode: process.env.STATUS_MODE || fileConfig.features?.statusMode ||
      "edit", // edit | send | off
    suppressProtocolSpam: /^(1|true|yes)$/i.test(
      process.env.SUPPRESS_PROTOCOL_SPAM || "",
    ),
    enableViewer: /^(1|true|yes)$/i.test(
      process.env.ENABLE_VIEWER || fileConfig.features?.enableViewer || "",
    ),
    enableWebServer: /^(1|true|yes)$/i.test(
      process.env.ENABLE_WEB_SERVER || fileConfig.features?.enableWebServer ||
        "",
    ),
    viewerPort: Number(
      process.env.VIEWER_PORT || fileConfig.features?.viewerPort || 3002,
    ),
    webServerPort: Number(
      process.env.WEB_SERVER_PORT || fileConfig.features?.webServerPort || 3001,
    ),
  },
};
...
export function validateConfig() {
  ...
  if (!config.discord.controlChannelId) {
    missing.push("DISCORD_CONTROL_CHANNEL_ID");
  }
