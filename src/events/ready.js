...
export default {
  ...
  async execute(client, context) {
    ...
    try {
      ...
      if (mode === 'edit') {
        statusMessage = await context.relayChannel.send('⏳ Initializing status ...');
      } else if (mode !== 'off') {
        await context.relayChannel.send('✅ Discord bot ready. Starting status loop.');
      }
      setInterval(async () => {
        if (mode === 'off') return;
        const st = getStatus();
        if (!st) return;
        const content = `Status | HP:${st.health} Food:${st.food} Ping:${st.ping} Pos:${st.coords}`;
        if (mode === 'edit') {
          if (!statusMessage) {
            statusMessage = await context.relayChannel.send(content).catch(()=>null);
          } else {
