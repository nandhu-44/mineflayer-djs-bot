...
export default {
  ...
  async execute(interaction, context) {
    ...
    if (interaction.isButton()) {
      ...
      if (!bot) return interaction.reply({ content: 'Bot offline', ephemeral: true });
      const moveDirs = ['forward','back','left','right','jump'];
      if (moveDirs.includes(id)) {
        bot.setControlState(id, true);
        setTimeout(() => bot.setControlState(id, false), id === 'jump' ? 500 : 1000);
        return interaction.reply({ content: `Moving ${id}`, ephemeral: true });
      }
      if (id === 'stop') {
        moveDirs.forEach(d => bot.setControlState(d, false));
        return interaction.reply({ content: 'Stopped movement', ephemeral: true });
      }
      if (id === 'eat') {
        try {
          const food = bot.inventory?.items()?.find(i => /bread|apple|carrot|potato|beef|pork|chicken|mutton|fish|stew/i.test(i.name));
          if (!food) return interaction.reply({ content: 'No food found', ephemeral: true });
          await bot.equip(food, 'hand');
          await bot.consume();
          return interaction.reply({ content: 'Ate food', ephemeral: true });
        } catch (e) {
          logger.error('Eat error', e);
          return interaction.reply({ content: 'Failed to eat', ephemeral: true });
        }
      }
      if (id === 'playerlist') {
    ...
    if (command.cooldown) {
      ...
      if (timestamps.has(interaction.user.id)) {
        ...
        if (now < expiration) {
          ...
          return interaction.reply({ content: `Cooldown: ${left}s left`, ephemeral: true });
        }
      }
    ...
    try {
      ...
      if (interaction.deferred || interaction.replied) {
        interaction.followUp({ content: 'Error executing command', ephemeral: true }).catch(()=>{});
      } else {
