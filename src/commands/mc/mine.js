...
export default new Command({
  category: "mc",
  data: new SlashCommandBuilder().setName("mine").setDescription(
    "Mine nearest block of a given type",
  )
    .addStringOption((o) =>
      o.setName("block").setDescription("Block name (e.g. oak_log)")
        .setRequired(true)
    ),
  async execute(interaction) {
    ...
    if (!blockId) {
      return interaction.reply({ content: "Unknown block", ephemeral: true });
    }
    const block = bot.findBlock({ matching: blockId, maxDistance: 64 });
    if (!block) {
      return interaction.reply({ content: "No block nearby", ephemeral: true });
    }
    try {
    ...
    try {
      ...
      interaction.reply({
        content: "Failed to mine: " + e.message,
        ephemeral: true,
      });
    }
  },
